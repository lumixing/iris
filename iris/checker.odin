...
check :: proc(top_stmts: []TopStmt) -> (err: Maybe(Error)) {
	info: Info
		...
		case Func:
			...
			for stmt in tstmt.body {
				...
				case Call:
					...
					if s.name in info.externs {
						name_found = true
					}

					if s.name in info.funcs {
						name_found = true
					}

					if !name_found {
						err = error({}, "Global %q is not defined", s.name)
						return
					}
				// case: unimplemented()
				}
